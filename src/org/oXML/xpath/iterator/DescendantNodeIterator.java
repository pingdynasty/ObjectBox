...
/**
 * overloads getNextNode() to return all descendants of the nodes of the parent iterator
 */
public class DescendantNodeIterator extends SubNodeIterator {
    protected Node last;
    protected Node top;

    public DescendantNodeIterator(NodeIterator root, NodeFilter filter){
        super(root, filter);
    }
	...
	// else get parent sibling while parent not last

	// get descendant
	Node next = last.getChildNodes().getNode(0);
	if(next != null){
	    last = next;
	    Log.trace("next3: "+last);
	    return last;
	}
	...
	do{
	    ...
	    if(next != null){
		Log.trace("next1: "+last);
		last = next;
		Log.trace("next2: "+last);
		return last;
	    }
