...
import org.oXML.type.NodeNode;
import org.oXML.type.StringNode;
import org.oXML.type.NumberNode;
import org.oXML.type.BooleanNode;

public class ObjectType extends ReflectionType{

    public static final Type TYPE = 
	new Type(new Name(ReflectionTypeResolver.JAVA_NS, "Object"));

    public ObjectType(ReflectionTypeResolver resolver){
        super(resolver, TYPE);
    }

    public Node primitive(Object object){
        // downcast to basic o:XML type or wrap in ReflectionNode
        if(object instanceof String)
            return new StringNode(object.toString());
        else if(object instanceof Number)
            return new NumberNode((Number)object);
        else if(object instanceof Boolean)
            return BooleanNode.booleanNode((Boolean)object);
