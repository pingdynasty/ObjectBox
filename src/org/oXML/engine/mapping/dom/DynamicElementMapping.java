...
import org.oXML.engine.template.DynamicElementTemplate;
import org.oXML.engine.CompilationContext;
import org.oXML.ObjectBoxException;
import org.w3c.dom.Element;

public class DynamicElementMapping implements TemplateMapping{
	...
	Resolver resolver = env.getResolver(e);
        ...
        Expression local = env.evaluate(name);
	local.bind(resolver);

        String ns = e.getAttribute("namespace");
        if(ns.equals("")){
            return new DynamicElementTemplate(local, body);
	}else{
	    Expression namespace = env.evaluate(ns);
	    namespace.bind(resolver);
            return new DynamicElementTemplate(namespace, local, body);
	}
    }
}
/*
